...
{
	$arr = explode(".",$file);//vì đuôi mở rộng nằm sau dấu chấm
...
function getPasswordRandom($n = 8) //Độ dài password
{
	...
	return substr(str_shuffle ($s), 0, $n);//đảo chuỗi với str_shuffle rồi cắt chuỗi từ vị trí 0 n kí tự
}
    ...
    <div class="info">
    	<?php 
			if (isset($_FILES["file"]) && ($_FILES["file"]["error"]==0))
				$ext = strtolower(getExt($_FILES["file"]["name"]));//lấy phần mở rộng viết thường
			else $ext="";//ko có file hình
